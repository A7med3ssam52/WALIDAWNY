...
| `set_platform_fee(numeric)` | ✗ | ✓ | مالك المنصة (mr_walid) أو admin داخل الدالة (رسوم ثابتة عامة) |
...
- `set_unit_price`: admin و mr_walid و teacher ينجحون في تحديد السعر الأساسي؛ `set_platform_fee`: مالك المنصة (mr_walid) أو admin ينجح، teacher → `permission_denied` (تعديل 0031 لقرار J، ثم 0033 — المدرس يحدد السعر الأساسي، والرسوم الثابتة للمالك/الأدمن).
