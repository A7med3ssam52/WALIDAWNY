...
> Scope: entire `src/` frontend of the منصة مستر وليد عونى التعليمية (React 19 + Vite 7 + Tailwind CSS v4 + react-router-dom 7).
...
7. **`index.html`** — `<title>منصة مستر وليد عونى التعليمية</title>` (English), no favicon, no `theme-color`; description is Arabic ✓.
